...
# Create tables if not exist (NOTE: does NOT migrate existing tables)
...
def cleanup_db():
    ...
    order_items -> orders -> items/users.
    """
    yield
...
def test_create_item_default_category():
    """
    New DB has category NOT NULL with default 'Головная боль'.
    If API ItemCreate has default too — category should be present in response.
    """
    response = client.post(
        "/api/items",
    ...
    # NEW: category should exist and be default
    assert "category" in data
    assert data["category"] == "Головная боль"

    # Image fields
    assert "has_image" in data
    assert data["has_image"] is False
    assert data["image_mime"] is None


def test_create_item_custom_category():
    response = client.post(
        "/api/items",
        json={
            "name": "Ibuprofen",
    ...
    assert data["category"] == "Жар"
    assert data["has_image"] is False


def test_list_items_contains_category():
    client.post("/api/items", json={"name": "Item 1", "price": 10.0})
    client.post("/api/items", json={"name": "Item 2", "price": 20.0, "category": "Другое"})
    ...
    assert len(items) >= 2
    assert all("category" in it for it in items)
    assert all("has_image" in it for it in items)


def test_get_item_contains_category():
    create_response = client.post(
        "/api/items",
...
def test_update_item_can_update_category_and_price_zero():
    """
    Verify update supports:
    - changing category
    - setting price to 0 (checks `is not None` logic)
    """
    create_response = client.post(
        "/api/items",
        json={"name": "Old Name", "price": 10.0, "category": "A"},
    )
    assert create_response.status_code == 201
    item_id = create_response.json()["item_id"]
    ...
    response = client.put(
        f"/api/items/{item_id}",
        json={"name": "New Name", "price": 0, "category": "B"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_upload_and_get_item_image():
    ...
    assert item_data["has_image"] is True
    assert item_data["image_mime"] == "image/png"
    assert "category" in item_data  # still present
...
def test_create_order():
    ...
    response = client.post(
        "/api/orders",
        json={"user_id": user_id, "items": [{"item_id": item_id, "quantity": 2}]},
    )
    assert response.status_code == 201
    data = response.json()
...
def test_get_order():
    ...
    order_response = client.post(
        "/api/orders",
        json={"user_id": user_id, "items": [{"item_id": item_id, "quantity": 1}]},
    )
